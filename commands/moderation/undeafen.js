...
export const aliases = ['undeaf', 'ud'];
export const usage = '!undeafen <@user> [reason]';
...
export async function execute(interaction) {
  ...
  try {
    ...
    await interaction.reply(modCard({ action: 'Undeafened', user, mod: interaction.user, reason }));
  } catch (e) {
...
export async function prefixExecute(message, args) {
  ...
  try {
    await member.voice.setDeaf(false, reason);
    await message.reply(modCard({ action: 'Undeafened', user: member.user, mod: message.author, reason }));
  } catch (e) {
